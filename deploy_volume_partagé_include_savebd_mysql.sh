...
if [ "$1" == "--create" ];then
  ...
  id_first=$(docker ps -a --format "{{ .Names }}" |grep "oki-vmparc" | sed s/".*-vmparc"//g  | sort -nr | head -1)
    ...
    docker run -v /srv/html:/var/www -v /srv/save_bd_mysql:/var/lib/mysql -tid --name ${USERNAME}-vmparc${i} madebian9_avec_v_p
    #docker run -v /srv/save_bd_mysql:/var/lib/mysql -tid --name ${USERNAME}-vmparc${i} madebian9_avec_v_p
